	...
	tests := []struct {
		...
		},
		{
			...
			want: []*ParseVar{
				{
					Id:     "var0",
					Filter: `count($serverHost='server.dabbler.com' $logfile='/var/log/nginx/access.log' $uriPath matches "\/r|\/click\.php" $status = 302 $redirectLocation matches "adssquared\.com")`,
					Query: &TimeseriesQuery{
						Filter:   `$serverHost='server.dabbler.com' $logfile='/var/log/nginx/access.log' $uriPath matches "\/r|\/click\.php" $status = 302 $redirectLocation matches "adssquared\.com"`,
						Function: "count",
						Priority: "low",
					},
					Response:   nil,
				...
				{
					Id:     "var1",
					Filter: `count($serverHost='ttt.server.com' $logfile='/var/log/nginx/access.log' $uriPath = "/fcc/" $status = 302 $redirectLocation matches "adssquared\.com")`,
					Query: &TimeseriesQuery{
						Filter:   `$serverHost='ttt.server.com' $logfile='/var/log/nginx/access.log' $uriPath = "/fcc/" $status = 302 $redirectLocation matches "adssquared\.com"`,
						Function: "count",
						Priority: "low",
					},
					Response:   nil,
				...
				{
					Id:     "var2",
					Filter: `count($serverHost='feeds.server.com' $logfile='/var/www/cfeeds.adsapphire.com/logs/feed_call_log.txt' $dataPostedDataServeUrl matches "dabbler\.com|realtor\.us|recipebook\.us|castle\.com|pets\.us|cars\.us|healthexchange\.org|lifestylealive\.com" $dataStatus == "ok" $dataPostedDataCachedAds == false)`,
					Query: &TimeseriesQuery{
						Filter:   `$serverHost='feeds.server.com' $logfile='/var/www/cfeeds.adsapphire.com/logs/feed_call_log.txt' $dataPostedDataServeUrl matches "dabbler\.com|realtor\.us|recipebook\.us|castle\.com|pets\.us|cars\.us|healthexchange\.org|lifestylealive\.com" $dataStatus == "ok" $dataPostedDataCachedAds == false`,
						Function: "count",
						Priority: "low",
					},
					Response:   nil,
